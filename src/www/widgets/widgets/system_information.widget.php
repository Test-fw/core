...
<script>
   ...
   */
   function system_information_widget_update(sender, data)
   {
      ...
      // MBUF memory usage New
      var tctx = $("#total_mem_usage")[0].getContext('2d');
      var total_mem = parseInt((parseInt(data['kernel']['mbuf']['total']) / parseInt(data['kernel']['mbuf']['max']))*100);
      ...
      var config = {
           ...
           data:{
            labels: ['Used', 'Available'],
             datasets: [{
               ...
               data: [total_mem, 100 - total_mem],
                backgroundColor: total_mem < 60 ? ['rgb(147,209,80)', ' rgb(217,217,217)'] : (total_mem < 80 ? ['rgb(237,124,48)', ' rgb(217,217,217)'] : ['rgb(255,0,0)', ' rgb(217,217,217)']),
                borderColor: total_mem < 60 ? ['rgba(0,255,0,0.8)', 'rgba(188,188,188,0.8)'] : (total_mem < 80 ? ['rgba(255,165,0,0.8)', 'rgba(188,188,188,0.8)'] : ['rgba(255, 2, 1, 0.8)', 'rgba(188,188,188,0.8)']),
               borderWidth: 1,
             }]
          },
            options: {
          ...
          if (window.totalUsageChart) {
            window.totalUsageChart.data = config.data;
            window.totalUsageChart.options = config.options;
            window.totalUsageChart.update();
          } else {
            window.totalUsageChart = new Chart(tctx, config);
          }
          window.totalUsageChart.canvas.parentNode.style.width = '160px'; 
          window.totalUsageChart.canvas.parentNode.style.height = '160px'; 
          window.totalUsageChart.canvas.style.width = '160px';
          window.totalUsageChart.canvas.style.height = '160px';
          $("#total_memory_display").html('<span style="font-family: SourceSansProSemibold;">'+(data['kernel']['mbuf']['total']/1024).toFixed(2)+'&nbspGB of&nbsp' +(data['kernel']['mbuf']['max']/1024).toFixed(2)+'&nbspGB Used</span>');
...
<table class="table table-striped table-condensed h-100" data-plugin="system" data-callback="system_information_widget_update">
  <tbody>
    ...
    <tr>
      <td style="height:150px; width:150px;">
        <div id="total_memory_usage" style="height:150px; width:150px;">   
          <canvas id="total_mem_usage"  width="150" height="150"></canvas>
        </div>
      </td>
    </tr>
    <tr>
...
<script>
  ...
  mCanv.style.margin = "auto";
  document.getElementById('system_memory_usage').appendChild(mCanv);
  var tCanv =  document.createElement("canvas");
  tCanv.setAttribute("id","total_mem_usage");
  tCanv.style.width = "150px";
  tCanv.style.height = "150px";
  tCanv.style.margin = "auto";
  document.getElementById('total_memory_usage').appendChild(tCanv);
</script>
